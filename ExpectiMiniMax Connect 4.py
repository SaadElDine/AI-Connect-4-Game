...
def getLastLocationMask(state, col):
    if state is not None:
        ...
        return -1  # Assuming a default value for an empty column
...
def getChildrenEXP(player, state):
    list = [33, 42, 24, 51, 15, 60, 6]
    children = []
    probs = [0.2, 0.6, 0.2]
    ...
    for i in range(0, 7):
        ...
        # Chosen column (0.6 probability)
        if temp != 7 and random.random() < 0.6:
            temp_state = state | (1 << (k - temp))
            temp_state = clear_bit(temp_state, k)
            temp_state = clear_bit(temp_state, k + 1)
            temp_state = clear_bit(temp_state, k + 2)
            ...
            if temp_state != state and temp_state not in children:
                children.append(temp_state)

        # Columns on the left and right (0.2 probability each)
        for offset in [-1, 1]:
            new_k = k + offset
            if 0 <= new_k <= 63:
                temp_adj = ((7 << new_k) & state) >> new_k
                if temp_adj == 7 and random.random() < 0.2:  # Check if the column is not already occupied
                    temp_state_adj = state | ((temp + 1) << new_k)

                    if temp_state_adj != state and temp_state_adj not in children:
                        children.append(temp_state_adj)

    return children


def expminiMax(maxDepth, depth, isMaxPlayer, state, heuristic):
    ...
    if isMaxPlayer:
        ...
        for child in children:
            childValue = expminiMax(maxDepth, depth + 1, not isMaxPlayer, child, heuristic)[1]
...
def nextMove(alphaBetaPruning, state, heuristic):  # The function returns the next best state in integer form
    ...
    if alphaBetaPruning:
        ans =expminiMax(BOARD.maxDepth, 0, True, state, heuristic)[0]
    else:
        ans =expminiMax(BOARD.maxDepth, 0, True, state, heuristic)[0]
...
# Initialize Window Size tuple 
WINDOW_SIZE = (WIDTH, HEIGHT)

# Initialize Customized Colors     
WHITE = (255, 255, 255)
...
#  @@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@
    def computerPlay(self):
        ...
        boardState = nextMove(alphaBetaPruning=False, state=numericState, heuristic=HEURISTIC_USED)
        ...
        # Check if newC is not None before using it in the loop
        if newC is not None:
...
class SettingsWindow:
    ...
    def drawSettingsMenuLabels(self):
        ...
        heuristicCaption = captionFont2_Arial.render("Choose Difficulty", True, BLACK)
